...
1. **Gemini CLI**
    * Option 1 - Install the Code Assist MCP server as a Gemini CLI extension. This provides the most complete experience, including specialized developer skills:
        ```bash
        gemini extensions install https://github.com/googlemaps/platform-ai.git
        ```
    * Option 2 - Add the MCP server config manually to your `~/.gemini/servers.json` file.
    ```json
    {
      "mcpServers": {
...
2. **Claude Code**
    * Add the server manually to your Claude config file `~/.claude.json`:
    ```json
    "mcpServers": {
        "google-maps-platform-code-assist": {
          "type": "sse",
